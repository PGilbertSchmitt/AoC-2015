  ...
  F: Fn(&str) -> bool,
{
  ...
  loop {
    let item = map.keys().sorted_unstable_by(|a, b| a.len().cmp(&b.len())).next().unwrap().clone();
    ...
    for (original_material, reduced_material) in lookup.iter() {
      if let Some(found_idx) = item.find(original_material) {
        let new_molecule = format!("{}{}{}", &item[0..found_idx], reduced_material, &item[found_idx+original_material.len()..]);
